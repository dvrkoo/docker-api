...
class MantraNet(nn.Module):
    def __init__(self, in_channel=3, eps=10 ** (-6), device=device):
        ...
        self.bayar_mask = (torch.tensor(np.ones(shape=(5, 5)), dtype=torch.float32)).to(
            self.device
        )
        self.bayar_mask[2, 2] = 0

        self.bayar_final = (
            torch.tensor(np.zeros(shape=(5, 5)), dtype=torch.float32)
        ).to(self.device)
        ...
        self.SRMConv2D.weight.data = torch.load(
            "trained_models/MantraNetv4.pt", weights_only=True
        )["SRMConv2D.weight"]
...
class MantraNet_GRU(nn.Module):
    def __init__(self, device, in_channel=3, eps=10 ** (-4)):
        ...
        self.SRMConv2D.weight.data = torch.load("MantraNetv4.pt", weights_only=True)[
            "SRMConv2D.weight"
        ]
...
def check_forgery(model, img_path=None, img=None, device="cpu"):
    # Prepare model
    model.to(device)
    model.eval()
    ...
    im = torch.Tensor(im).unsqueeze(0).transpose(2, 3).transpose(1, 2).to(device)

    with torch.no_grad():
        final_output = model(im)

    # Extract outputs
    forgery_mask = final_output[0][0].cpu().detach().numpy()
    suspicious_regions = (
        final_output[0][0].cpu().detach().unsqueeze(2).numpy() > 0.2
    ) * original_image
    ...
    return {
        ...
        "suspicious_regions": suspicious_regions.astype(np.uint8),
    }
